...
{
  if (tail.empty()) { return std::vector<Position>(); }
  return std::vector<Position>(tail.begin(), tail.end() - 1);
}

void Player::UpdateDirection()
{
  if (IsNewDirectionValid()) { currentDirection = nextDirection; }
}
...
void Player::MoveTail()
{
  if (!shouldAddSegment)
  {
    if (!tail.empty()) { tail.erase(tail.begin()); }
  }
...
{
  if (position.x <= CANVAS_BOUNDS.xMin) { return true; }

  if (position.x >= CANVAS_BOUNDS.xMax) { return true; }

  if (position.y <= CANVAS_BOUNDS.yMin) { return true; }

  if (position.y >= CANVAS_BOUNDS.yMax) { return true; }
